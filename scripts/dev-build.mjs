...
async function syncStaticAddonArtifacts() {
    ...
    // PulseSync дописывает в metadata.json свои поля (например `installSource`).
    // Их нужно сохранить, иначе сборка перезапишет аддон как «магазинный».
    ...
    await fs.writeFile(metadataPath, JSON.stringify({ ...existing, ...metadata }, null, 4) + '\n', 'utf8')
}
